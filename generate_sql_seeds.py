...
with open("all_data_export.json", "r", encoding="utf-8") as f:
    data = json.load(f)
...
for cat, items in products_data.items():
    for idx, p in enumerate(items):
        ...
        sql_lines.append(f"""insert into public.products (slug, category, group_name, label, name, detail, specs, image_url, sort_order, published)
values ({sql_quote(slug)}, {sql_quote(cat)}, {sql_quote(group)}, {sql_quote(label)}, {sql_quote(name)}, {sql_quote(detail)}, {sql_quote(specs)}, {sql_quote(image_url)}, {idx}, true)
...
sql_lines.append(f"\n-- TOTAL PRODUCTS SEEDED: {prod_count}\n")
...
art_count = 0
for idx, a in enumerate(articles_data):
    art_count += 1
    slug = a["slug"]
    cat = a["category"]
    title = a["title"]
    ...
    sql_lines.append(f"""insert into public.articles (slug, category, title, excerpt, hero_image, sections, references_list, published)
values ({sql_quote(slug)}, {sql_quote(cat)}, {sql_quote(title)}, {sql_quote(excerpt)}, {sql_quote(hero_image)}, {sql_quote(sections)}, {sql_quote(references)}, true)
on conflict (slug) do update set
  category = excluded.category,
...
sql_lines.append(f"\n-- TOTAL ARTICLES SEEDED: {art_count}\n")

with open("SUPABASE_SETUP.sql", "w", encoding="utf-8") as f:
    f.write("\n".join(sql_lines))

print(f"Successfully generated SUPABASE_SETUP.sql with {prod_count} products and {art_count} articles!")
